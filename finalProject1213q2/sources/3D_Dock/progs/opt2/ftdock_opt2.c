...
struct Atom_Charged *charged_atoms;
int num_charged_atoms = 0; // determines the size of previous array
...
/************/
  ...
  if (gettimeofday(&start, NULL) == -1) exit(0);
...
/************/
  ...
  posix_memalign((void **)&charged_atoms, 16, num_charged_atoms * sizeof_Atom_Charged);

/************/

  /* Store new structures centered on Origin */

  Origin_Static_Structure = translate_static_structure_onto_origin( Static_Structure, num_charged_atoms, charged_atoms ) ;
...
/************/
  ...
  if( electrostatics == 1 ) {
    electric_field( Origin_Static_Structure , grid_span , global_grid_size , static_elec_grid, num_charged_atoms, charged_atoms ) ;
...
/************/
  ...
  if (gettimeofday(&end, NULL) == -1) exit(0);
