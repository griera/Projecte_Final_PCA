...
/************/
  ...
  for( x = 0 ; x < grid_size ; x ++ ) {
	...
	for( atoms = 0; atoms < num_charged_atoms; atoms ++ ) {
        float xyz_sqrt[4];
	 __m128 punt = _mm_load_ps((float *)&charged_atoms[atoms]); 
         __m128 res = _mm_sub_ps(punt, centre);
	res = _mm_mul_ps(res, res);
	_mm_store_ps(&xyz_sqrt[0], res); // xyz_sqrt[0] conté basura! (no vàlid per als càlculs) 
	distance = sqrt(xyz_sqrt[1] + xyz_sqrt[2] + xyz_sqrt[3]); // sqrt(x, y, z)
